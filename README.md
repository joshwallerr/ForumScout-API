# ForumScout API Documentation

Welcome to the ForumScout API documentation. This guide explains how to integrate with our API, manage your API key, and understand usage limits and billing. 

## Table of Contents

1. [Introduction](Introduction.md)
2. [Getting Started](GettingStarted.md)
3. [Authentication](Authentication.md)
4. [API Endpoints](APIEndpoints.md)
    - [Forum Search](ForumSearch.md)
    - [API Key Management](APIKeyManagement.md)
5. [Code Examples](CodeExamples.md)
6. [Usage Limits and Billing](UsageLimitsBilling.md)
7. [FAQ and Support](FAQ.md)
8. [Contact and Support](ContactSupport.md)
